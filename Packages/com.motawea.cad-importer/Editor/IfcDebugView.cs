...
namespace CADImporter.Editor
{
    ...
    /// <see cref="IfcElement"/> data the importer attaches. Overrides are applied through
    ...
    /// <see cref="Clear"/>. Separated from the window so it is testable headlessly.
    /// </summary>
    public static class IfcDebugPainter
    {
        ...
        /// <summary>Removes every override under <paramref name="root"/>.</summary>
    ...
    /// and triangle statistics per category. Clicking a legend row selects those elements.
    /// All colouring is a transient MaterialPropertyBlock override — nothing is modified.
    /// </summary>
    public class IfcDebugWindow : EditorWindow
    {
        ...
        void DrawLegend()
        {
            ...
            foreach (var g in groups)
            {
                ...
                var swatch = new Rect(row.x + 2, row.y + 3, 14, 14);
                EditorGUI.DrawRect(swatch, g.Color);

                float pct = totalTris > 0 ? 100f * g.Triangles / totalTris : 0f;
                var label = new Rect(row.x + 22, row.y, row.width - 22, row.height);
                GUI.Label(label,
                    $"{g.Label}   —   {g.Elements.Count} elem, {g.Triangles:N0} tris ({pct:F1}%)");

                // Click a row to select its elements in the Hierarchy.
            ...
            EditorGUILayout.HelpBox(
                "Colours are transient MaterialPropertyBlock overrides — the scene, prefab and " +
                "imported assets are untouched. Click a legend row to select its elements.",
                MessageType.None);
        }
    }
}
